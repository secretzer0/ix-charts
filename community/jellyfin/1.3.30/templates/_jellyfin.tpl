    ...
    podSpec:
      ...
      containers:
        jellyfin:
          ...
          env:
            # HOME must point at the writable config volume: the server (and dotnet)
            # falls back to $HOME paths for stray writes, and the root FS is read-only.
            HOME: /config
            # Pin the config dir explicitly. StartupHelpers' XDG fallback only honors
            # the --datadir CLI flag (not JELLYFIN_DATA_DIR), so a fresh volume without
            # a config/ subdir would otherwise resolve config to $HOME/.config/jellyfin.
